...
def main(argv):
    ...
    try:
       ...
       print('generateEQFiles.py -y <year> -m <month> -d <day> -s <start> -e <end>')
       sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('generateEQFiles.py -y <year> -m <month> -d <day> -s <start> -e <end>')
    ...
    if start > 0:
        ...
        while year <= end:
            month = 1
            while month <= 12:
                state = generateEQFile(year, month, day)
                if state == -1:
                    return
                month = month + 1
            year = year + 1
...
def generateEQFile(year, month, day):
    ...
    for stock_data_obj in stock_data_list:
        ...
        print(s_month)
        print(s_date)
        bhavcopy_save(date(int(s_year),int(s_month),int(s_date)), '/home/surya/git/customPrograms/eqFiles/')
        time.sleep(7)
