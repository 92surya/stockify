...
def main(argv):
    ...
    try:
       ...
       print('generateFOFiles.py -y <year> -m <month> -d <day> -s <start> -e <end>')
       sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('generateFOFiles.py -y <year> -m <month> -d <day> -s <start> -e <end>')
    ...
    if start > 0:
        ...
        while year <= end:
            month = 1
            while month <= 12:
                state = generateFOFile(year, month, day)
                if state == -1:
                    return
                month = month + 1
            year = year + 1
...
def generateFOFile(year, month, day):
    ...
    with open(filename, newline='') as csv_file:
        ...
        for trade_date,series,open_price,high,low,prev_close,ltp,close,vwap,w52_h,w52_l,volume,value,no_of_trades,symbol in reader:
            ...
            stock_data_list.append(StockData(trade_date,series,open_price,high,low,prev_close,ltp,close,vwap,w52_h,w52_l,volume,value,no_of_trades,symbol))
    ...
    for stock_data_obj in stock_data_list:
        ...
        # if s_date == 30:
        #     continue
        bhavcopy_fo_save(date(int(s_year),int(s_month),int(s_date)), '/home/surya/git/customPrograms/foFiles/')
        time.sleep(7)
